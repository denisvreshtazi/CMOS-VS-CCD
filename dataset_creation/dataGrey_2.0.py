...
for filename in os.listdir(os.path.join(DIR, 'CCD')):
...
a = {'Name': img, 'Camera': camera, 'PathGrey': directory, 'Sensor': sensor}
A = pd.DataFrame(a)
...
for filename in os.listdir(os.path.join(DIR, 'CMOS')):
...
b = {'Name': img1, 'Camera': camera1, 'PathGrey': directory1, 'Sensor': sensor1}
B = pd.DataFrame(b)

data = pd.concat([A,B], ignore_index=True)
path = input("Directory to save: ")
data.to_csv(path+'dataGrey.csv')
